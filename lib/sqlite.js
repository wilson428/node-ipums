...
module.exports = function(opts) {
	var codebook = fs.readFileSync(opts._[1] + ".cbk", "utf8"),
		...
		db = new sqlite3.Database(opts._[1] + ".sqlite");

	var table  = opts._[1].split("/").slice(-1)[0];
	...
	var command = "INSERT INTO " + table + " (" + fields.join(",") + ") VALUES (" + fields.map(function(d) { return "$" + d; }).join(",") + ");";
	...
	db.serialize(function() {
		...
		rl.on('line', function(line) { 
			...
			if (!tabled) {
				var create_table = "CREATE TABLE " + table + " (" + fields.map(function(d) { 
					return d + " " + js_to_sqlite[typeof datum["$"+d].val]; 
				}).join(", ") + ")";
				console.log("hello");
				db.run(create_table);
				tabled = true;
